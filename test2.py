...
# Sinon on la cree
	...
	for i in range(0,len(dir)):
		...
		if os.path.exists('desc' + str(i)):
			print("descriptors already computed in 'desc" + str(i) + "'")
		...
		# S'ils n'existent pas deja, on les calcule
			...
			for f in path(dire).walkfiles():
				if f.endswith('jpg') and f!='NULL':
					...
					try:
						...
						k=k+1
						sys.stdout.write('\r' + 'chargement : ' + str(k) + '/' + str(tai))
						sys.stdout.flush()
						# Fin chargement
			...
			print("descripteurs de la classe enregistres dans 'desc'" + str(i))

	# Chargement des descripteurs enregistres en concatenation
	for i in range(0,len(dir)):
		desfinal = np.append(desfinal, cPickle.load(open("desc" + str(i))))

	# Preparation de la matrice de descripteurs pour le Kmeans
	desc = np.reshape(desfinal, (len(descriptors)/128, 128))
